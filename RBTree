public class RBTree {
  ...
  public void delete(Uzel y){   //удаление
    Uzel t;
    Uzel q;
  ...
  public void delete_balance(Uzel x){
    while(x!=root && x.color == "BLACK"){
      if(x == x.parent.left){
        ...
        if(k.color == "RED"){ // случай 2(презентация)
          k.color = "BLACK";
          k.parent.color = "RED";
          rotateL(x.parent);
          k = x.parent.right;
  ...
  public void put_balance(Uzel t){ //балансируем после добавления
    while (t!= root && t.parent.color == "RED"){ //если отец красный и узел не корень
      if(t.parent == t.parent.parent.left){ //если отец в левом поддереве деда 
        uncle = t.parent.parent.right; //дядя правый 
        ...
        if(t.parent == t.parent.parent.right){ // тогда дядя в левой ветке деда
          ...
          else{ // иначе если дядя черного цвета
            if(t == t.parent.left){
